...
export default async function postData(req:NextApiRequest,res:NextApiResponse){
    const db = firebase_admin.firestore();
    const sessionCookie=req.cookies.sessionCookie;
    const decodedClaims = await firebase_admin.auth().verifySessionCookie(sessionCookie, true);
